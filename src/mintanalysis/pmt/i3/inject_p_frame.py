...
class WaveFormUnfold(pone_unfolding.WaveUnfold):
    ...
    def Physics(self,frame):
        ...
        # Tricky: Delete waveforms for channels not currently being used, to avoid needing calibration for them.
        raw_data = copy.copy(frame["RawData"])
        for c in [c for c in raw_data.keys() if c not in channels]:
            del raw_data[c]
...
def main():
    ...
    parser.add_argument("-u", "--upsample", type=int, default=100, help="Upsample factor (spe per bin)")
    parser.add_argument("-t", "--tolerance", type=float, default=2, help="Chi^2 stopping tolerance")
    parser.add_argument("-w", "--waveform", default="", help="Name of refolded waveform field (no waveforms stored if omitted or empty string)")
    ...
    msg = f"Creating P frame. This may take a while"
    logger.info(msg)
    with heartbeat_ctx():
        tray = icetray.I3Tray()
        tray.AddModule('I3Reader', 'reader',filename=args.f_i3_in)
        tray.AddModule(injectEmptyPhysics)
        tray.AddModule(WaveFormUnfold,SPEsPerBin=args.upsample, OutputWaveforms="", RefoldedWaveforms=args.waveform, Tolerance=args.tolerance)

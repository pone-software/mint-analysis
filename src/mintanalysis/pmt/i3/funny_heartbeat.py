...
def funny_line(elapsed_seconds: float) -> str:
    ...
    line = f"{random.choice(VERBS)} {random.choice(OBJECTS)} {random.choice(PHYSICS)}"
    line += f" ({random.choice(methods_pool)}). {random.choice(endings_pool)}"
...
def heartbeat_ctx(line_interval: float = 10.0, spinner_interval: float = 0.1):
    ...
    t = threading.Thread(
        target=funny_heartbeat, args=(stop, line_interval, spinner_interval), daemon=True
    )
    t.start()
    try:
        yield
